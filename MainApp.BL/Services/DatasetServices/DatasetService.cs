...
namespace MainApp.BL.Services.DatasetServices
{
    public class DatasetService : IDatasetService
    {
        ...
        public async Task<EditDatasetDTO> GetObjectForEditDataset(Guid datasetId, string? searchByImageName, bool? searchByIsAnnotatedImage, bool? searchByIsEnabledImage, string? orderByImages, int pageNumber, int pageSize)
        {
            ...
            var imageResult = await _datasetImagesRepository.GetAll(x => x.DatasetId == datasetId, null, includeProperties: "ImageAnnotations") ?? throw new Exception("Object not found");
        ...
        public async Task<ResultDTO<DatasetDTO>> ImportDatasetCocoFormatedAtDirectoryPath(
             string datasetName,
             string cocoDirPath,
             string userId,
             string? saveDir = null,
             bool allowUnannotatedImages = false)
        {
            ...
            try
            {
                ...
                foreach (CocoCategoryDTO category in cocoDataset.Categories)
                {
                    ...
                    {
                        datasetClassEntity = existingDatasetClass.Data; // Use existing class
                    }
                    else
                    {
                        ...
                        await _datasetClassesRepository.Create(datasetClassEntity); // Save new class
                    }
