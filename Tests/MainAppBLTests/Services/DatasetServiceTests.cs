...
namespace Tests.MainAppBLTests.Services
{
    public class DatasetServiceTests
    {
        ...
        [Fact]
        public async Task GetDatasetById_ShouldReturnFailResult_WhenDatasetNotFound()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            ...
            Assert.Equal($"Dataset not found, for id: {datasetId}", result.ErrMsg);
        }


        [Fact]
        public async Task GetDatasetById_ReturnsErrorResult_WhenDatasetNotFound()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            ...
            //Assert
            Assert.Equal("Failed to retrieve dataset.", result.ErrMsg);
        }

        [Fact]
        public async Task GetDatasetById_RetunsErrorResult_WhenMappingFails()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var dataset = new Dataset { Id = datasetId };
        ...
        public async Task CreateDataset_ShouldReturnDatasetDto_WhenCreationIsSuccessful()
        {
            ...
            // Assert
            Assert.NotNull(result);
            Assert.Equal(datasetDto.Id, result.Data.Id);
        }

        [Fact]
        public async Task CreateDataset_ShouldReturnFail_WhenCreatedDatasetIsNull()
        {
            // Arrange
            var datasetDto = new DatasetDTO { Id = Guid.NewGuid() };
            var dataset = new Dataset { Id = datasetDto.Id };

            _mockMapper.Setup(mapper => mapper.Map<Dataset>(It.IsAny<DatasetDTO>())).Returns(dataset);
            _mockDatasetRepository.Setup(repo => repo.CreateAndReturnEntity(It.IsAny<Dataset>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(ResultDTO<Dataset>.Ok(null));
            ...
            // Assert
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to create dataset. Dataset is null.", result.ErrMsg);
        }
        [Fact]
        public async Task CreateDataset_ShouldReturnFail_WhenMappingFails()
        {
            // Arrange
            var datasetDto = new DatasetDTO { Id = Guid.NewGuid() };
            var dataset = new Dataset { Id = datasetDto.Id };
            ...
            _mockDatasetRepository.Setup(repo => repo.CreateAndReturnEntity(It.IsAny<Dataset>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(ResultDTO<Dataset>.Ok(dataset));
            _mockMapper.Setup(mapper => mapper.Map<DatasetDTO>(It.IsAny<Dataset>())).Returns((DatasetDTO)null);
        ...
        public async Task CreateDataset_ShouldThrowException_WhenCreationFails()
        {
            ...
            _mockDatasetRepository
                .Setup(repo => repo.CreateAndReturnEntity(It.IsAny<Dataset>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(ResultDTO<Dataset>.Fail("Failed to create dataset."));
        ...
        [Fact]
        public async Task AddDatasetClassForDataset_ShouldReturnFailure_WhenClassIsNotAddedSuccessfully()
        {
            // Arrange
            var selectedClassId = Guid.NewGuid();
            var datasetId = Guid.NewGuid();
        ...
        [Fact]
        public async Task GetAllDatasets_ReturnsFailResult_WhenNoDatasetsFound()
        {
            // Arrange
            _mockDatasetRepository
                .Setup(repo => repo.GetAll(null, null, false, "CreatedBy,UpdatedBy,ParentDataset", null))
                .ReturnsAsync(ResultDTO<IEnumerable<Dataset>>.Fail("Dataset not found"));
        ...
        [Fact]
        public async Task GetAllPublishedDatasets_ShouldReturnFailResult_WhenNoDatasetFound()
        {
            // Arrange
            _mockDatasetRepository
                .Setup(repo => repo.GetAll(It.IsAny<Expression<Func<Dataset, bool>>>(), null, false, "CreatedBy,ParentDataset", null))
                .ReturnsAsync(ResultDTO<IEnumerable<Dataset>>.Ok(null));
        ...
        [Fact]
        public async Task GetDatasetDTOFullyIncluded_ShouldReturnFailResult_WhenDatasetNotFound()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
        ...
        public async Task GetObjectForEditDataset_ShouldReturnEditDatasetDTO_WhenSuccessful()
        {
            ...
            // Assert
            Assert.NotNull(result);
            Assert.IsType<ResultDTO<EditDatasetDTO>>(result);
        }

        [Fact]
        public async Task GetObjectForEditDataset_ShouldReturnFailResult_WhenDatasetDatasetClassesIsNull()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            _mockDatasetDatasetClassRepository
                .Setup(repo => repo.GetAll(null, null, false, "DatasetClass,Dataset", null))
                .ReturnsAsync(ResultDTO<IEnumerable<Dataset_DatasetClass>>.Fail("Failed to retrieve dataset classes associated with the dataset."));
            ...
            Assert.NotNull(result);
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to retrieve dataset classes associated with the dataset.", result.ErrMsg);
        }

        [Fact]
        public async Task GetObjectForEditDataset_ShouldReturnFailResult_WhenDatasetClassesIsNull()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            ...
            _mockDatasetDatasetClassRepository
                .Setup(repo => repo.GetAll(null, null, false, "DatasetClass,Dataset", null))
                .ReturnsAsync(ResultDTO<IEnumerable<Dataset_DatasetClass>>.Ok(datasetDatasetClasses));
            _mockDatasetClassesRepository
                .Setup(repo => repo.GetAll(null, null, false, "ParentClass,Datasets", null))
                .ReturnsAsync(ResultDTO<IEnumerable<DatasetClass>>.Fail("Failed to retrieve available dataset classes."));
            ...
            Assert.NotNull(result);
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to retrieve available dataset classes.", result.ErrMsg);
        }

        [Fact]
        public async Task GetObjectForEditDataset_ShouldReturnFailResult_WhenCurrentDatasetIsNull()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            ...
            _mockDatasetRepository
                ...
                .ReturnsAsync(ResultDTO<Dataset>.Fail("Failed to retrieve current dataset."));
            _mockDatasetClassesRepository
                .Setup(repo => repo.GetAll(null, null, false, "ParentClass,Datasets", null))
            ...
            Assert.NotNull(result);
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to retrieve current dataset.", result.ErrMsg);
        }

        [Fact]
        public async Task GetObjectForEditDataset_ShouldReturnFailResult_WhenNumberOfImagesToPublishIsNull()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            ...
            _mockAppSettingsAccessor
                ...
                .ReturnsAsync(ResultDTO<int>.Fail("Failed to retrieve number of images needed to publish the dataset."));
            ...
            Assert.NotNull(result);
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to retrieve number of images needed to publish the dataset.", result.ErrMsg);
        }

        [Fact]
        public async Task GetObjectForEditDataset_ShouldReturnFailResult_WhenNumberOfClassesToPublishIsNull()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var dataset = new Dataset();
            ...
            _mockAppSettingsAccessor
                ...
                .ReturnsAsync(ResultDTO<int>.Fail("Failed to retrieve number of classes needed to publish the dataset."));
            ...
            Assert.NotNull(result);
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to retrieve number of classes needed to publish the dataset.", result.ErrMsg);
        }

        [Fact]
        public async Task GetObjectForEditDataset_ShouldReturnFailResult_WhenImageResultIsNull()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            ...
            _mockDatasetImagesRepository
                ...
                .ReturnsAsync(ResultDTO<IEnumerable<DatasetImage>>.Fail("Failed to retrieve dataset images."));
            ...
            Assert.NotNull(result);
            Assert.False(result.IsSuccess);
            Assert.Equal("Failed to retrieve dataset images.", result.ErrMsg);
        }
        ...
        public async Task GetObjectForEditDataset_ShouldFilterImages_ByImageName()
        {
            ...
            // Assert
            Assert.NotNull(result);
            Assert.All(result.Data.ListOfDatasetImages, image => Assert.Equal("test-image", image.Name));
        }
        ...
        public async Task GetObjectForEditDataset_ShouldFilterImages_ByIsEnabled()
        {
            ...
            // Assert
            Assert.NotNull(result);
            Assert.All(result.Data.ListOfDatasetImages, image => Assert.True(image.IsEnabled));
        }
        ...
        public async Task GetObjectForEditDataset_ShouldFilterImages_ByIsAnnotated()
        {
            ...
            // Assert
            Assert.NotNull(result);
            Assert.NotEqual(0, result.Data.NumberOfAnnotatedImages);
        }
        ...
        public async Task GetObjectForEditDataset_ShouldReturnCorrectEditDatasetDTO_NoFilters()
        {
            ...
            Assert.Equal(1, result.Data.NumberOfDatasetClasses);
            Assert.Equal("MappedDataset", result.Data.CurrentDataset.Name);
            Assert.Equal(2, result.Data.NumberOfDatasetImages);
            Assert.Equal(1, result.Data.NumberOfClassesNeededToPublishDataset);
            Assert.Equal(100, result.Data.NumberOfImagesNeededToPublishDataset);
            Assert.Equal(1, result.Data.NumberOfEnabledImages);
            Assert.Equal(2, result.Data.NumberOfAnnotatedImages);
            Assert.False(result.Data.AllEnabledImagesHaveAnnotations);
            Assert.NotEmpty(result.Data.ListOfAllDatasetImagesUnFiltered);

            Assert.Equal("MappedClass", result.Data.UninsertedDatasetRootClasses.First().ClassName);
            Assert.Single(result.Data.UninsertedDatasetSubclasses);
        }
        ...
        [Fact]
        public async Task PublishDataset_ShouldReturnFailure_WhenDatasetNotFound()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var userId = "user-id";
            ...
            Assert.Equal("Dataset data could not be retrived", result.ErrMsg);
        }

        [Fact]
        public async Task PublishDataset_ShouldReturnFailure_WhenNoDatasetImages()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var userId = "user-id";
            ...
            Assert.False(result.IsSuccess);
            Assert.Equal(3, result.Data);
            Assert.Equal("Dataset images could not be retrived", result.ErrMsg);
        }

        [Fact]
        public async Task PublishDataset_ShouldReturnFailure_WhenNoEnabledImages()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var userId = "user-id";
            var dataset = new Dataset { Id = datasetId };
            var datasetImages = new List<DatasetImage> { new DatasetImage { IsEnabled = false }, new DatasetImage { IsEnabled = false } };
        ...
        [Fact]
        public async Task PublishDataset_ShouldReturnFailure_WhenNoInsertedClasses()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var userId = "user-id";
            var dataset = new Dataset { Id = datasetId };
            var datasetImages = new List<DatasetImage> { new DatasetImage { IsEnabled = true, Id = Guid.NewGuid() } };
            ...
            _mockImageAnnotationsRepository
                .Setup(repo => repo.GetAll(null, null, false, null, null))
                .ReturnsAsync(ResultDTO<IEnumerable<ImageAnnotation>>.Ok(new List<ImageAnnotation> { new ImageAnnotation { DatasetImageId = datasetImages[0].Id } }));
            ...
            Assert.False(result.IsSuccess);
            Assert.Equal(3, result.Data);
            Assert.Equal("No inserted classes were found", result.ErrMsg);
        }


        [Fact]
        public async Task PublishDataset_ShouldReturnFailure_WhenDataIsInsufficientClassesOrImages()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var userId = "user-id";
            var dataset = new Dataset { Id = datasetId };
            var datasetImages = new List<DatasetImage> { new DatasetImage { IsEnabled = true }, new DatasetImage { IsEnabled = true }, new DatasetImage { IsEnabled = true } };
            var datasetClasses = new List<Dataset_DatasetClass> { new Dataset_DatasetClass { DatasetId = datasetId, DatasetClass = new DatasetClass() }, new Dataset_DatasetClass { DatasetId = datasetId, DatasetClass = new DatasetClass() } };
            ...
            _mockImageAnnotationsRepository
                .Setup(repo => repo.GetAll(null, null, false, null, null))
                .ReturnsAsync(ResultDTO<IEnumerable<ImageAnnotation>>.Ok(new List<ImageAnnotation> { new ImageAnnotation() }));

            _mockAppSettingsAccessor
                .Setup(accessor => accessor.GetApplicationSettingValueByKey<int>("NumberOfImagesNeededToPublishDataset", 100))
                .ReturnsAsync(ResultDTO<int>.Ok(50));

            _mockAppSettingsAccessor
                .Setup(accessor => accessor.GetApplicationSettingValueByKey<int>("NumberOfClassesNeededToPublishDataset", 1))
                .ReturnsAsync(ResultDTO<int>.Ok(5));
            ...
            var datasetImages = new List<DatasetImage>
    {
        ...
        [Fact]
        public async Task SetAnnotationsPerSubclass_ShouldReturnFailure_WhenDatasetIsNull()
        {
            // Arrange
            var datasetId = Guid.NewGuid();
            var userId = "test-user-id";
        ...
        [Fact]
        public async Task DeleteDatasetClassForDataset_ShouldReturnFailure_WhenDatasetClassDataIsNull()
        {
            // Arrange
            var selectedClassId = Guid.NewGuid();
            var datasetId = Guid.NewGuid();
            ...
            _mockDatasetDatasetClassRepository
                .Setup(repo => repo.GetFirstOrDefault(It.IsAny<Expression<Func<Dataset_DatasetClass, bool>>>(), false, null))
                .ReturnsAsync(ResultDTO<Dataset_DatasetClass?>.Ok(null));
            ...
            Assert.False(result.IsSuccess);
            Assert.Equal(2, result.Data);
            Assert.Equal("Unable to retrieve data for the specified dataset class.", result.ErrMsg);
        }

        [Fact]
        public async Task DeleteDatasetClassForDataset_ShouldReturnFailure_WhenDatasetDataIsNull()
        {
            // Arrange
            var selectedClassId = Guid.NewGuid();
            var datasetId = Guid.NewGuid();
